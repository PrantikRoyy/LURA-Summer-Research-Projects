...
execute(addParticipant(P,M),_) :- format("Participant ~w added to ~w~n",[P,M]).

% Executes the sendInvitationEmail action by displaying that an invitation email was sent.
execute(sendInvitationEmail(CA,P,M),_) :- format("~w sending invitation email to ~w for ~w~n",[CA,P,M]).

% Executes the queryConstraints action by displaying the participant's email response.
execute(queryConstraints(_CA,P,_M,EC),_) :- format("~nEmail response received from ~w:~n",[P]), format("  \"~w\"~n",[EC]).

% Executes the acquireDelegatedConstraints action by displaying that a delegated task was acquired.
...
% Initializes the meeting and registers all participants.
proc(initializeMeeting,
    [
        createMeeting(meeting1),
        addParticipant(alice,meeting1),
        addParticipant(bob,meeting1),
        addParticipant(charlie,meeting1)
    ]).

% Simulates Alice submitting her availability response.
proc(simulateResponse(alice,meeting1),
    injectExog(
        responds(alice,meeting1,
...
% Organizes a meeting by collecting constraints and then
% determining suitable meeting times.
proc(meetingOrganized(SA,M),
    [constraintsCollected(SA,M),
     meetingTimeIdentified(SA,M)]).

% Chooses one available method for collecting constraints.
proc(constraintsCollected(SA,M),
    ndet(
        constraintsCollectedByEmail(SA,M),
        constraintsCollectedByDoodle(SA,M)
    )).

% Collects participant constraints through email until every
% participant has extracted scheduling constraints.
proc(constraintsCollectedByEmail(_SA,M),
    while(some(P, and([participant(P,M), neg(some( EC, extractedConstraints(ca,P,M,EC)))])),
          pi(P, [?(and([participant(P,M), neg(some(EC, extractedConstraints(ca,P,M,EC)))])),
          individualConstraintsCollectedByEmail(ca,P,M)]))).

% Placeholder for a Doodle-based collection method.
% Currently disabled.
proc(constraintsCollectedByDoodle(_SA,_M),
    ?(false)).
...
% Determines the meeting time after all constraints have
% been collected.
...
% Main program executed by the IndiGolog interpreter.
% First initializes the meeting, then performs scheduling.
proc(main,
    [
        initializeMeeting,
