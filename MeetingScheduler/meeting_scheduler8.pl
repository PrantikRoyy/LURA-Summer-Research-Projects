...
execute(createMeeting(M),_) :- format(">> createMeeting(~w): Meeting created: ~w~n",[M,M]).

% Executes the addParticipant action by displaying that a participant was added.
execute(addParticipant(P,M),_) :- format(">> addParticipant(~w,~w): Participant ~w added to ~w~n", [P,M,P,M]).

% Executes the sendInvitationEmail action by displaying that an invitation email was sent.
execute(sendInvitationEmail(CA,P,M),_) :- format(">> sendInvitationEmail(~w,~w,~w): ~w sending invitation email to ~w for ~w~n", [CA,P,M,CA,P,M]).

% Executes the queryConstraints action by displaying the participant's email response.
execute(queryConstraints(CA,P,M,EC),_) :-
    format(">> queryConstraints(~w,~w,~w,\"~w\"): Displaying the participant's email response.~n",
           [CA,P,M,EC]),
    format("   Email response received from ~w: ",[P]),
    format("   \"~w\"~n",[EC]).
...
execute(acquireDelegatedConstraints(CA,P,M),_) :-
    format(">> acquireDelegatedConstraints(~w,~w,~w): ~w acquired delegated task for ~w in ~w~n",
           [CA,P,M,CA,P,M]).

% Executes a simulated participant response by injecting it as an
% exogenous event into the IndiGolog event queue.
execute(injectExog(responds(P,M,EC)),_) :- format("<< responds(~w,~w,\"~w\"): Participant submitted an email response.~n", [P,M,EC]),
    exog_queue_add(responds(P,M,EC)).

% Executes simulated constraint extraction by injecting the extracted
% scheduling constraints as an exogenous event.
execute(injectExog(extractConstraints(CA,P,M,EC,LA,LP,LI)),_) :-
    format("<< extractConstraints(~w,~w,~w,\"~w\",~w,~w,~w): Scheduling constraints extracted from the email.~n",
           [CA,P,M,EC,LA,LP,LI]), 
    exog_queue_add(extractConstraints(CA,P,M,EC,LA,LP,LI)).

% Fallback for any other exogenous event.
execute(injectExog(Event),_) :- format("<< ~q: External event occurred.~n",[Event]), exog_queue_add(Event).
...
execute(reportPreferredSlotsAction(M),H) :-
    format(">> reportPreferredSlotsAction(~w): Reporting all collected scheduling constraints and preferences.~n", [M]),
...
proc(simulateResponse(alice,meeting1), injectExog(responds(alice,meeting1, "Available Monday/Tuesday. Prefer Monday morning."))).

% Simulates Bob submitting his availability response.
proc(simulateResponse(bob,meeting1), injectExog(responds(bob,meeting1, "Tuesday afternoon works best."))).

% Simulates Charlie submitting his availability response.
proc(simulateResponse(charlie,meeting1), injectExog(responds(charlie,meeting1, "Only Wednesday if needed."))).
...
proc(main, [initializeMeeting, meetingOrganized(sa,meeting1)]).
