...
/* Defines p1-p3 as valid packages as facts */
package(p1).
package(p2).
package(p3).
...
proc(handle_packages(Max),
    ndet(
        ...
        [/* Check that every package has been picked */
         ?(and(picked(p1) = true, and(picked(p2) = true, picked(p3) = true))),
        ...
        [/* Get the robot's current position */
         ...
         ?(picked(p1) = S1), ?(picked(p2) = S2), ?(picked(p3) = S3),
         ...
         ?((pairs_from_status([p1-PX1-PY1-S1, p2-PX2-PY2-S2, p3-PX3-PY3-S3], RX, RY, Pairs), Pairs \= [],
             sort(Pairs, [D-P|_]))),
...
[
    ...
    /* While the robot is still carrying at least one package */
    while(neg(and(carrying(p1) = false, and(carrying(p2) = false, carrying(p3) = false))),
        ...
        ndet(
            /* If carrying p1, drop it off */
            [?(carrying(p1) = true), dropoff(p1)],
            ndet(
                /* If carrying p2, drop it off */
                [?(carrying(p2) = true), dropoff(p2)],
                /* If carrying p3, drop it off */
